...
| **UPD-1017 → UPD-1017R** | ~~Wrap webhooks~~ **REFRAMED to webhook integration placeholders / disabled-state** per owner (providers not configured yet) | 🟡 **PR #18 + PR #19 both CLOSED (superseded, not merged; branches deleted). Surviving implementation = Supercharged [PR #20](https://github.com/ratchetnu/supercharged/pull/20) `codex/operion-upd-1017-integration-readiness`, head `f502726`, NOT merged (ready for owner merge decision).** Assigned **Session 3**. Full approved scope: `.env.example` (names-only) + `!.env.example` gitignore + owner/session-gated `GET /api/admin/integrations` + Integrations status on the EXISTING Settings page + Twilio/email fail-closed 503 guards + centralized `app/lib/integrations.ts` + tests. Stripe untouched (already 503). Reconciled the one unique behavior from #19 (whitespace-only values treated as absent). Webhook **tenant wrapping DEFERRED**. |
...
### UPD-1017R result (Session 3, 2026-07-23) — SUPERSEDES PR #18; IMPLEMENTED, HELD FOR REVIEW

**⚠️ Owner clarification superseded the PR #18 approach.** Stripe/Twilio/inbound email are **not configured yet** and must remain **disabled placeholders** until setup later. **PR #18 CLOSED (not merged); its branch `codex/operion-upd-1017-webhook-tenant-wrap` deleted (local + remote); the active Stripe tenant wrapper is NOT preserved.** Since #18 was never merged, SC `main` never carried the wrapper — nothing to revert.

**Replacement: Supercharged [PR #19](https://github.com/ratchetnu/supercharged/pull/19)**, branch `codex/operion-webhook-integration-placeholders`, head `a112d46`, base SC `main` `ab0cead`, **MERGEABLE, NOT merged.** Exactly **5 files (+301/−0)**:
- **`app/lib/platform/integrations.ts`** (new) — single source of truth: **names-only, presence-only** `configured`. Never reads/returns a value.
- **`app/admin/operations/settings/integrations/page.tsx`** (new) — read-only server component (behind `/admin`): name + configured boolean + required field **names** + "setup after build"; **never a value; never claims connected/ready/active/verified**.
- **`app/api/webhooks/twilio/sms/route.ts`** + **`app/api/webhooks/email/route.ts`** — disabled-state guards: no-op **before** any body read / provider call / redis / message / opt-out / booking / owner-alert.
- **`scripts/integration-readiness.test.ts`** (new) — 9 tests.

**Stripe route UNCHANGED** — on `main` it already returns `503 stripe not configured` before processing. **No tenant code, no fallback tenant, no flag/env change, no registration/dispatch.** Proof of no-mutation-while-unconfigured: the webhook tests drive the real `POST` handlers with a request whose body readers **throw**; each returns its inert response **without throwing** → the guard short-circuits before the body read (all collaborators sit after it).

**Verification (SC `main` `ab0cead`):** focused **9/9**; UPD-1016 gate **55/55**; `booking-payments` **11/11**; full suite **542→551**; tsc + ESLint clean; **build ✓135/135** (new page dynamic); CI `regression` SUCCESS. SC `main` still `ab0cead`; no dispatch; archived UPD-1004 untouched. **Webhook tenant wrapping is DEFERRED** (prerequisites: providers configured + records carry `tenantId` + stores adopt `scopeKey`).

---

<details><summary>Superseded — original UPD-1017 (PR #18) audit record, kept for history</summary>

**Supercharged PR #18** (CLOSED), branch `codex/operion-upd-1017-webhook-tenant-wrap`, head `1160b3a`. Webhook family audited (exactly 3 under `app/api/webhooks/`):
...
| 2026-07-23 | S3 | **UPD-1017 REFRAMED → UPD-1017R (webhook integration placeholders / disabled-state) per owner clarification.** Providers (Stripe/Twilio/email) are not configured yet → must remain disabled placeholders. **PR #18 CLOSED (superseded, not merged); branch `codex/operion-upd-1017-webhook-tenant-wrap` deleted; active Stripe wrapper NOT preserved** (never merged → nothing to revert). **Replacement Supercharged [PR #19](https://github.com/ratchetnu/supercharged/pull/19), head `a112d46`, NOT merged (held for independent review).** 5 files (+301/−0): readiness module (names-only, presence-only) + read-only admin setup page (never values/never connected-ready-active-verified) + Twilio/email disabled-state guards (no-op before body read) + tests. Stripe route UNCHANGED (already 503s unconfigured). Proof of no mutation: poison-request tests — handlers return inert without reading the body. No tenant code, no fallback tenant, no flag/env change, no registration/dispatch. Verify: focused 9/9, UPD-1016 gate 55/55, booking-payments 11/11, full 542→551, tsc+eslint clean, build ✓135/135, CI regression SUCCESS. SC main still `ab0cead`; UPD-1004 untouched. Webhook tenant wrapping DEFERRED. **UPD-1018–1023 remain BLOCKED; IMG-002 + UPD-1002 remain parked.** |
