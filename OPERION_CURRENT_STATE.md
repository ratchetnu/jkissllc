...
| Booking-backed claims | Completed, partially completed, and could-not-complete customer bookings can open claims from the booking detail. The server freezes the booking customer, service date, job address, net invoice, disposal, active crew, and crew-pay snapshot; declined crew are excluded and caller-supplied snapshot fields are ignored. Responsibility and deductions remain a separate explicit admin step, and posted deductions flow through the existing claim ledger into the same pay statement as booking earnings. |
...
| 10 | **Booking pay/hours reached payroll but claims did not — FIXED 2026-08-21.** Claims now accept a server-resolved `bookingToken`, freeze customer/job/crew/money data through `snapshotFromBooking()`, and remain linked by booking token/number in the admin booking and claim views. Forged customer, crew, booking number, and snapshot money fields are ignored; mixed route+booking identity is rejected; unfinished bookings cannot freeze a premature claim snapshot. Opening a claim assigns no responsibility and deducts nothing. The existing explicit responsibility, deduction-plan, accrual, and immutable pay-statement path is reused for both work lanes. |
| 12 | **Release Center claimed "Update available" against an UNKNOWN installed version — FIXED 2026-07-25.** Supercharged rendered `Current version —` beside *"A newer version (0.1.0) is available"*, and the activity list filled with five identical "Update available" rows (one per 15-minute `operion-reconcile` pass). Cause: the projection trusted a platform-sync `updateAvailable` flag while `currentBaselineVersion` was empty — a system cannot truthfully claim it is behind when it does not know what it is running. Fixed by centralising derivation in `deriveVersionState()` (`release/versions.ts`), which fails **closed** to `version_unknown` and only reports `update_available` for a KNOWN installed version that is genuinely older; a new `version_unknown` release status renders *"Version unknown"* / *"Not yet deployed through Operion."* in neutral tone, never the attention badge. Consecutive identical activity rows are collapsed. **Supercharged's installed Operion version remains genuinely unknown** and was NOT backfilled: `deriveBusinessProvenance()` writes `currentVersion`/`latestVerifiedVersion` only from an associated `releaseVersion`, and Supercharged has never completed one — **UPD-1007 was a canary (1 file, promotion intentionally not requested, PR closed unmerged) and did not establish an installed product version.** It will stay unknown until the first verified successful deployment through Operion. |
| 11 | **Retry policy did not consult the update's status — FIXED 2026-07-25.** `retryPreview()` checked the job (status, target, flags) but never `update.status`, so the `update_approved` gate that blocks archived updates — which lives only in `evaluatePreflight()`, called by `preparePreview()` and *not* by `retryPreview()` — was bypassed on every retry. Three disagreeing job-status sets existed (`orchestrator.RETRYABLE`, `deploy-view.OWNER_RETRYABLE`, `progress.RETRYABLE`) and the correct helper `isOwnerRetryable()` had **no callers at all**. Consequence: **UPD-1004 — archived and terminal — was re-fired from attempt 4 to 5** (Supercharged workflow run `30135658768`, 2026-07-25T00:10:26Z, `failure`), from the Release Center's Retry button, whose copy read *"You can retry safely."* **This was NOT the cron reconciler**: `/api/cron/operion-reconcile` auto-retries only `TRANSIENT_FAILURES` (`timeout`, `provider_error`, `internal_error`) bounded at 2 attempts, and `apply_failed` is in neither. Owner retries were also **unbounded** — nothing capped `attemptCount`. Fixed by centralising eligibility in `retryEligibility()` (deploy-view), shared by the dispatcher, the button, and its copy; it requires the update to be in `APPROVED_STATUSES`, fails closed on an unknown status, and caps owner retries at `MAX_OWNER_RETRIES`. **Archived updates are now terminal and cannot be re-fired.** A blocked retry dispatches nothing and does not increment `attemptCount`. |
...
3. **Booking-backed claims shipped 2026-08-21** — no remaining claims/booking integration blocker. Continue to require an explicit admin responsibility split and deduction plan before any contractor pay is reduced.
...
2. **Completed 2026-08-21:** claims read booking assignments and freeze the customer-job snapshot; booking earnings and posted claim deductions reconcile on the same pay statement (§4 #10).
