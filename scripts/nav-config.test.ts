...
test('desktop mega-menu categories are ordered + non-empty, with the reference grouping', () => {
  ...
  assert.deepEqual(byKey('team'), ['/admin/operations/users', '/admin/operations/permissions', '/admin/operations/audit', '/admin/operations/crew-activity'], 'Team & Access group: users + permissions + audit + crew activity')
...
test('mobile More sheet leads with ONE Team section holding Crew and Team & Access', () => {
  ...
  assert.deepEqual(groups[0].items.map(i => i.href), ['/admin/operations/employees', '/admin/operations/users', '/admin/operations/permissions', '/admin/operations/audit', '/admin/operations/crew-activity'])
