...
test('builds a valid manifest from a commit: add + modify + delete, with content + hashes', async () => {
  ...
  const r = await buildCommitTransferManifest({ provider, installationId: '146887383', sourceRepo: REPO, sourceRepoName: 'ratchetnu/jkissllc', sourceCommit: 'deadbeef', updateKey: 'UPD-1004', target: TARGET })
...
test('fails cleanly when a source file cannot be read', async () => {
  ...
  const r = await buildCommitTransferManifest({ provider, installationId: '1', sourceRepo: REPO, sourceRepoName: 'ratchetnu/jkissllc', sourceCommit: 'abc', updateKey: 'UPD-1', target: TARGET })
...
test('fails when the update has no source commit', async () => {
  const provider = mockProvider([], {})
  const r = await buildCommitTransferManifest({ provider, installationId: '1', sourceRepo: REPO, sourceRepoName: 'ratchetnu/jkissllc', sourceCommit: '', updateKey: 'UPD-1', target: TARGET })
  assert.equal(r.ok, false)
})
