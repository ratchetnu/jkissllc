...
export function assignedSms(c: ClaimRecord, a: ClaimAssignment): string {
  const s = safe(c, a)
  return `${COMPANY.shortNameUpper}: A damage claim (${s.claimNumber}) from ${s.businessName} has been assigned to you. Your share is ${money(a.responsibilityCents)}. We'll go over it with you — reply or call with questions.`
}

export function deductionStartedSms(c: ClaimRecord, a: ClaimAssignment): string {
  const s = safe(c, a)
  const weekly = a.weeklyDeductionCents ?? 0
  return `${COMPANY.shortNameUpper}: Starting ${money(weekly)}/week toward claim ${s.claimNumber} (${s.businessName}) from the week of ${a.startDate}. Balance: ${money(remainingCents(a))}. Each deduction shows on your pay statement.`
}

export function deductionChangedSms(c: ClaimRecord, a: ClaimAssignment): string {
  const s = safe(c, a)
  const weekly = a.weeklyDeductionCents ?? 0
  return `${COMPANY.shortNameUpper}: Your weekly deduction for claim ${s.claimNumber} (${s.businessName}) is now ${money(weekly)}/week. Balance: ${money(remainingCents(a))}.`
}

export function deductionCompleteSms(c: ClaimRecord, a: ClaimAssignment): string {
  const s = safe(c, a)
  return `${COMPANY.shortNameUpper}: Claim ${s.claimNumber} (${s.businessName}) is paid off. No further deductions. Thank you.`
}

export function balanceWaivedSms(c: ClaimRecord, a: ClaimAssignment): string {
  const s = safe(c, a)
  return `${COMPANY.shortNameUpper}: The remaining balance on claim ${s.claimNumber} (${s.businessName}) has been waived. No further deductions.`
}
