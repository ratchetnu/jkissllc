...
export type DiscoveredConfig = { installationId: string; repositoryOwner: string; repositoryNameOnly: string; repoName: string; defaultBranch: string }
...
export async function validateGithubConnection(b: PlatformBusiness, env: Record<string, string | undefined> = process.env): Promise<ValidateResult> {
  ...
  // Canonical owner/name from the saved record (repoName preferred, owner+name fallback).
  const repo = businessRepoRef(b)
  push('Repository configured', !!repo, repo ? `${repo.owner}/${repo.name}` : 'set the repository as "owner/name" (e.g. ratchetnu/supercharged)')
  ...
  const discovered: DiscoveredConfig | undefined = repoRes.ok ? { installationId, repositoryOwner: repo.owner, repositoryNameOnly: repo.name, repoName: `${repo.owner}/${repo.name}`, defaultBranch } : undefined
