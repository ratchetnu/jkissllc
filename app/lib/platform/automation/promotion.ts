...
export function canAutoRollback(input: { status: string; flagEnabled: boolean; eligible?: boolean; rollbackTargetDeploymentId?: string; attemptCount?: number; maxAttempts?: number }): PromotionGateResult {
  ...
  if (input.eligible !== true) return { ok: false, reason: 'this job was not prepared with a verified automatic rollback path' }
